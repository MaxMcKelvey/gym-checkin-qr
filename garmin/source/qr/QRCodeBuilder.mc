...
class QRCodeBuilder {
    ...
    private const mMode as Mode = BINARY;
    ...
    private var mProgress as Number?;
    ...
    private function _progress() as Float {
        ...
        var progressStep = mProgress != null ? mProgress : 0;
        return ((progressStep.toFloat() / mStepsToFinish) * 100) + subProgress;
    }
    ...
    private function _iterateAddData() as Void {
        switch (mProgress) {
            ...
            case ADD_DATA_ENCODE: {
                $.log("ADD_DATA_ENCODE");
                // Payload uses '@' and digits; byte mode is required (not alphanumeric).
                _encodeBinary();
                break;
            }
            ...
            case ADD_DATA_DATA_BLOCKS: {
                ...
                if (!mProgressPayload.hasKey(:current)) {
                    ...
                    var errorInfo = QRCodeTables.eccwbi[mVersion][QRCodeTables.error[mError]] as Numbers;
    ...
    private function _grouped(chunkSize as Number, items as Array, fillValue as Lang.Object?) as Array {
